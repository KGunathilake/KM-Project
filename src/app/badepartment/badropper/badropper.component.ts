...
@Component({
  selector: 'app-badropper',
  templateUrl: './badropper.component.html',
  styleUrls: ['./badropper.component.css']
})
export class BadropperComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<BadropperComponent>,
...
async fileUpload(event: FileList) {
  ...
  const fileStoragePath = `File/BADepartment/${new Date().getTime()}_${file.name}`;
...
createdoc(id,data){
  this.api.createfile(id,data,'BADepartment').then(data=>{
