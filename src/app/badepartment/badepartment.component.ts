...
@Component({
  selector: 'app-badepartment',
  templateUrl: './badepartment.component.html',
  styleUrls: ['./badepartment.component.sass']
})
export class BadepartmentComponent implements OnInit {
...
openUplaod(): void {
    const dialogRef = this.dialogModel.open(BadropperComponent, {
      width: '600px',
      disableClose: true
    });
  ...
  getfilessuc(){
   ...
   this.api.getfilechanges('BADepartment').subscribe(data=>{
    this.files = data;
    this.updateResults();
    this.updateResults();
  ...
  deletedoc(docid){
    this.api.deletedoc(docid,'BADepartment').then(data=>{
