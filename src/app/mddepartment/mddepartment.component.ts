...
import { MddropperComponent } from './mddropper/mddropper.component';

@Component({
  selector: 'app-mddepartment',
  templateUrl: './mddepartment.component.html',
  styleUrls: ['./mddepartment.component.sass']
})
export class MddepartmentComponent implements OnInit {
...
openUplaod(): void {
    const dialogRef = this.dialogModel.open(MddropperComponent, {
      width: '600px',
      disableClose: true
    });
  ...
  getfilessuc(){
   ...
   this.api.getfilechanges('MDDepartment').subscribe(data=>{
    this.files = data;
    this.updateResults();
    this.updateResults();
  ...
  deletedoc(docid){
    this.api.deletedoc(docid,'MDDepartment').then(data=>{
