...
@Component({
  selector: 'app-mddropper',
  templateUrl: './mddropper.component.html',
  styleUrls: ['./mddropper.component.css']
})
export class MddropperComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<MddropperComponent>,
...
async fileUpload(event: FileList) {
  ...
  const fileStoragePath = `File/MDDepartment/${new Date().getTime()}_${file.name}`;
...
createdoc(id,data){
  this.api.createfile(id,data,'MDDepartment').then(data=>{
