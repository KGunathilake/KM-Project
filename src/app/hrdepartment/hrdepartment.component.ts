...
@Component({
  ...
  styleUrls: ['./hrdepartment.component.css']
})
...
openUplaod(): void {
    const dialogRef = this.dialogModel.open(DropperComponent, {
      width: '600px',
      disableClose: true
    });
  ...
  getfilessuc(){
   ...
   this.api.getfilechanges('HRDepartment').subscribe(data=>{
    this.files = data;
    this.updateResults();
    this.updateResults();
  ...
  deletedoc(docid){
    this.api.deletedoc(docid,'HRDepartment').then(data=>{
