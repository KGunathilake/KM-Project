...
@Component({
  selector: 'app-dddropper',
  templateUrl: './dddropper.component.html',
  styleUrls: ['./dddropper.component.css']
})
export class DddropperComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DddropperComponent>,
...
async fileUpload(event: FileList) {
  ...
  const fileStoragePath = `File/DDDepartment/${new Date().getTime()}_${file.name}`;
...
createdoc(id,data){
  this.api.createfile(id,data,'DDDepartment').then(data=>{
