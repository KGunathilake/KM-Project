...
@Component({
  selector: 'app-dddepartment',
  templateUrl: './dddepartment.component.html',
  styleUrls: ['./dddepartment.component.sass']
})
export class DddepartmentComponent implements OnInit {
...
openUplaod(): void {
    const dialogRef = this.dialogModel.open(DddropperComponent, {
      width: '600px',
      disableClose: true
    });
  ...
  getfilessuc(){
   ...
   this.api.getfilechanges('DDDepartment').subscribe(data=>{
    this.files = data;
    this.updateResults();
    this.updateResults();
  ...
  deletedoc(docid){
    this.api.deletedoc(docid,'DDDepartment').then(data=>{
